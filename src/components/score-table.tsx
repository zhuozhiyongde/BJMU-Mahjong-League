...
export function ScoreTable({ members, onRename, onDelete, lastUpdateTime, importEnabled = true }: ScoreTableProps) {
    const columnCount = 16;
    return (
        <Card>
            <CardHeader className="pb-3">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    <CardTitle>BJMU League Season4 成绩表</CardTitle>
                    {lastUpdateTime && (
                        <span className="text-sm text-muted-foreground">最后更新: {lastUpdateTime}</span>
                    )}
                </div>
            </CardHeader>
            <CardContent>
